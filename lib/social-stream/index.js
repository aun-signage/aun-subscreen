...
var SocialStream = module.exports = function(env, pgClient) {
  ...
  var sweepThrottled = _.throttle(sweep, 1000);

  var store = function(type, time, payload, text) {
    pgClient.query(
      'INSERT INTO messages (type, time, payload, text) VALUES ($1, $2, $3, $4)',
      [type, time, payload, text],
      function(err, result) {
        if (err) {
  ...
  var receiveTwitter = function() {
    ...
    tw.on('tweet', function(data) {
      ...
      store('tweet', new Date(), data, data.text);
    });
  };
  ...
  var receiveIrc = function() {
    ...
    irc.on('message', function(message) {
      ...
      store('irc', new Date(), message, message.message);
    });
  };
