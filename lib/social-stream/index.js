...
var SocialStream = module.exports = function(env, pgClient) {
  ...
  var sweep = function(limit) {
    pgClient.query(
      'SELECT time FROM messages ORDER BY time DESC LIMIT $1',
      [limit],
      function(err, result) {
        if (err) {
          throw err;
        }
        if (result.rowCount > 0 && result.rowCount == limit) {
          var oldestRow = result.rows[result.rowCount - 1];
          var timeThreshold = oldestRow.time;
          pgClient.query(
